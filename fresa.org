...
#+begin_src julia :eval no :tangle example.jl
  <<exampleusingfresa>>
      <<exampleobjective>>
      <<exampledomain>>
...
- February 2023 :: *v8.0*, major upgrade with possible *breaking* changes where I have refactored the calling sequences for some functions, specifically:
  ...
  2. Removed the need to pass a ~Domain~ object to the ~neighbour~ function as there are problems which are unbounded or for which the bounds are implicitly encoded in the decision variables.  This is a breaking change for any case where problem specific ~neighbour~ functions have been defined.

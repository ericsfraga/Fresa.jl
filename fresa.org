...
* TODO bugs and new features [0/3]                            :noexport:
- [ ] use constraint information in fitness calculations
- [ ] allow for user specified /neighbour/ function
- [ ] implement MINLP example
...
Please let [[mailto:e.fraga@ucl.ac.uk?subject=The%20Strawberry%20code][the author know]] if you download the code.  Further, if either /Fresa/ or /Strawberry/ codes are used, please do let the authors know and the [[file:strawberry.org][publications]] must be cited in any research papers written.  Feedback, including bug reports, is most welcome.
...
#+begin_src julia
  function fitness(pop)
      ...
      fit = zeros(l,1)
      feasiblefit = vectorfitness(m, feasible[n+1:n+m,:])
      if length(indexinfeasible) > 0
          # scale up feasible solutions fitness
          feasiblefit = feasiblefit/2 + 0.5 # upper half of fitness internval
          infeasiblefit = vectorfitness(m, infeasible[n+1:n+m,:])/2; # lower half
          fit[indexfeasible] = feasiblefit
          fit[indexinfeasible] = infeasiblefit
      else
          fit = feasiblefit
      end
      fit
...
A random solution is generated with a distance from the original point being inversely proportional, in a stochastic sense, to the fitness of the solution.  The new point is possibly adjusted to ensure it lies within the domain defined by the lower and upper bounds.
#+name: neighbour
#+begin_src julia
...
#+begin_src julia
  ...
  b = [6.0; 6.0]
  Fresa.setbounds(a,b)
  p = Fresa.randompopulation(10,f,a,b)
#+end_src 

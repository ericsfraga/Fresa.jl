...
Given the above code, Fresa can now be used to solve the problem.  The ~solve~ function has many options (see [[solve]] section below).  Besides the lower and upper bounds, there is one option that should be specified: a stopping criterion for the search.  This may be either the number of generations to perform, ~ngen~, or the number of objective function evaluations to allow, ~nfmax~.  If no stopping criterion is specified, a warning message will be generated and a default value will be used.  Here, we specify 100 generations to perform.

The other option that is often specified is the number of solutions to propagate from one generation to the next, ~np~.  This has a default value which is reasonable for many problems so we will simply accept that default.
...
- (TBD) :: *v8.2*, minor release providing support for unbounded search domains.  See [[neighbour]] section below for a description of the use of /mapping/ from an unbounded domain to a bounded domain.
...
#+begin_src julia :var version=(org-sbe tomlversion)
  version = "8.2.0"
  lastchange = "[2024-04-04 15:12+0100]"

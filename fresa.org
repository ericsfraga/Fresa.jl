...
#+property: header-args :comments yes :eval no-export
...
#+end_src
The ~Fresa.createpoint~ function expects two arguments: the actual point in the search domain and the actual objective function.
...
#+end_src
Should other decision point types be required, e.g. mixed-integer or domain specific data structures, the =Fresa.neighbour= function with parameters of the specific type will need to be defined.  See the [[*mixed integer problems][mixed integer problems]] below for an example of a simple mixed-integer case.

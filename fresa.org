...
* TODO bugs and new features [8/13]                           :noexport:
...
#+end_src
Should other decision point types be required, e.g. mixed-integer or domain specific data structures, the =Fresa.neighbour= function with parameters of the specific type will need to be define.  See the [[*mixed integer nonlinear example][mixed integer nonlinear example]] below for an example of a simple mixed-integer case.
          ...
          for i in 1:min(l,npop)
              ...
              for r in 1:nr
                  ...
                  if parallel
                      ...
                      push!(newpop, Point(newx, f, parameters))
                      nf += 1
                  end
              end

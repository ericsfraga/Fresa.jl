...
* TODO bugs and new features [2/7]                            :noexport:
- [ ] current version requires =f= to return a vector for objective function value.  A single objective function should not require this so need to catch this case.
...
#+begin_src julia
  type Point
      ...
      function Point(x, f)
          (z, g) = f(x)
          if typeof(z) == Array{Float64}
              new(x,z,g)
          elseif typeof(z) == Float64
              new(x,[z],g)
          else
              println("Objective function values should be Float64 values, not $(typeof(z))")
          end
      end
  end
#+end_src 
...
The objective function, =f=, should return two results: =z=, the objective vn value(s) which must be of type =Float64=, single or array, and =g=, the constraint violation.  If =g≤0=, the point is feasible; any value =g>0= means an infeasible point.  The value of =g= for infeasible points will be used to rank the fitness of the infeasible solution, with lower values being fitter.

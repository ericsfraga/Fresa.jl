...
#+begin_src julia
  function fitness(pop)
      ...
      if 1 == m
          ...
          error("Fresa not implemented for multi-objective optimisation, nz=$M, yet.")
          z = pop[n+1:n+m,:]
          ...
          fitness = map(x->prod(x), rank[:,i] for i=1:5)
...
#+begin_src julia
  function solve(f,x0,a,b)
      ...
      index = sortperm(fit)
      best = pop[:,index[length(fit)]]
      best[1:n], best[n+1:n+m], best[n+m+1], pop
  end
#+end_src 
** module end
#+name: moduleend

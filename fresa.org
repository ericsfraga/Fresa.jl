...
#+begin_src julia
  function fitness(pop)
      ...
      if length(indexinfeasible) > 0
          ...
          fit[indexinfeasible] = vectorfitness(1, infeasible[n+m+1:n+m+1,:])/factor;
      end
      vec(fit)
  end
#+end_src 
...
#+begin_src julia
  function solve(f,x0,a,b;
      ...
      pop = reshape([x0;z;g],n+m+1,1) # should not be necessary
      for gen in 1:ngen
          fit = fitness(pop)
          index = sortperm(fit)

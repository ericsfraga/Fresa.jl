...
#+begin_src julia
  function fitness(pop,nx)
      ...
      if length(indexfeasible) > 0
          ...
          feasiblefit = vectorfitness(m, feasible[nx+1:nx+nz,:])
...
#+begin_src julia
  function neighbour(x,nx,a,b,f)
      xnew = x .+ (1-f) * 2(rand(nx,1)-0.5) .* (b-a)
  ...
  function solve(f,x0,a,b;
      ...
      for gen in 1:ngen
          ...
          for i in 1:min(l,npop)
              ...
              for r in 1:nr
                  x = neighbourfn(pop[1:n,s],nx,a,b,fit[s])
                  z, g = f(x)
      ...
      if m == 1
          best = pop[:,index[length(fit)]]
          return best[1:nx], best[nx+1:nx+nz], best[nx+ny+1], pop
      else
...
#+begin_src julia :tangle testminlp.jl
  ...
  function neighbour(x,nx,a,b,f)
      ...
      newx = zeros(2,1)
      newx = x .+ (b-a)*(1-f) * 2 * (rand(nx,1)-0.5)
      newx[2] = round(newx[2])
      newx[newx.<a] = a
      newx[newx.>b] = b
      return newx

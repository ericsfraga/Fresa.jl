...
# citations
#+cite_export: csl /usr/share/texlive/texmf-dist/tex/latex/citation-style-language/styles/chicago-author-date.csl
...
- /Optimization of a PID controller within a dynamic model of a steam Rankine cycle with coupled energy storage/ [cite:@ward-etal-2023b].
...
#+begin_src julia :var version=(org-sbe tomlversion)
  lastchange = "[2025-11-20 16:35+0000]"
...
Optional arguments:
- analysePopulation :: if this option is set to a function (default ~nothing~), with signature

  ~(Int, Array{Point}, Array{Float64}, Array(Point), Array{Float64})~,

  the function will be called at the start of each generation with the generation number, the set of non-dominated points in the population, and the set of dominated points.  For single objective problems, the non-dominated set will consist of one point, the best in the population.

  An [[*rosenbrock][example test case]] shows how this can be used.
...
#+end_src
** rosenbrock
This problem is continuous and optimization methods often find it difficult /[to] converge to the global minimum/ [[https://en.wikipedia.org/wiki/Rosenbrock_function][Wikipedia]].  We consider the default case often used to test out optimization methods.

In this case, we also use the ability to analyse the population at start of each generation to demonstrate that capability.  The example is trivial, simply identifying at which generation the best solution was obtained.
...
#+name: gamsmodel
#+begin_src gams :tangle "test/gamsdeclarations.gms" :comments no
  $TITLE Test Problem 
  $OFFDIGIT
  $OFFSYMXREF 
  $OFFSYMLIST 

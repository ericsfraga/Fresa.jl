...
* TODO bugs and new features [8/11]                           :noexport:
...
*** an objective function with interface to GAMS 
#+name: gamsf
#+begin_src julia
  function f(x::Array{Float64,1})
      ...
      open("gamsoutput.txt", "r") do f
          ...
          z[2] = parse(Float64, lines[2])
          modelstat = parse(Float64, lines[3])
          if modelstat != 2 && modelstat != 5
              g = 1
          end
      end
      # return results
...
*** test GAMS example objective function 
#+name: gamstest
#+begin_src julia 
  (z, g) = f([1.0;2.0;3.0])
  println("z = $z and g = $g")

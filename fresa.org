...
#+end_src
* TODO bugs and new features [14/30]                          :noexport:
- [ ] consider generalising the ~neighbour~ function to work with unbounded search domains (or semi-infinite domains) by mapping the decision variables to (-π/2,+π/2) through use of the ~atan~ function (and mapping back to the infinite domain with ~tan~).

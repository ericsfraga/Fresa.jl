...
* TODO bugs and new features [8/9]                            :noexport:
...
** Installation
This document has been written using [[http://orgmode.org/][org mode]] in the [[https://www.gnu.org/software/emacs/][Emacs]] text editor.  Org allows for /literate programming/ and uses /tangling/ to generate the actual source files for the code.  The code, tangled from this file, can be found here: call_distribution().  To use this code, you could add this to your =~/.juliarc.jl= file (for Linux; for MS Windows, check the [[http://www.julialang.org/][Julia]] documentation):
#+begin_src julia
...
#+begin_src julia
  ...
  using Fresa
#+end_src 
Although ~Fresa~ has no external dependencies, the [[*Tests][test problems below]] may require one or more of the following packages: ~BenchmarkTools~, ~PyPlot~.
...
#+begin_src julia
  """
  Point (`x`) in the search space along with objective function 
  values (`z[]`) and feasbility indication (`g`).  The type of `x` is
  problem specific.  `z[]` and `g` hold `Float64` values.
  """
  type Point
      ...
      if l == 1
          ...
          if m == 1                   # single objective 
              ...
              elseif fitnesstype == :borda
                  ...
                  throw(ArgumentError("Type of fitness evaluation must be either :hadamard or :borda, not $(repr(fitnesstype))."))
              end
          end
      end
      # println("VF: fit=$fit")
...
#+begin_src julia
  ...
  function solve(f, x0, a, b;     # required arguments
      ...
      @printf(": solving with ngen=%d npop=%d nrmax=%d ns=%d\n",
              ngen, npop, nrmax, ns)
      ...
      for gen in 1:ngen
          ...
          if elite
              if nz > 1
                  ...
                  wholepareto = pareto(pop)
                  if length(wholepareto) > ceil(npop/2)
                      newpop = thinout(pop, fit, wholepareto, ceil(Int,npop/2))
                  else
          ...
          for i in 1:min(l,npop)
              ...
              for r in 1:nr
                  ...
                  newx = neighbourfn(pop[s].x,a,b,fit[s])
          ...
          if parallel
              ...
              while i < length(x)
                  ...
                  for j=1:nprocs()
                      if i+j <= length(x) 
                          results[j] = @spawn createpoint(x[i+j],f) #remotecall(createpoint,j,x[i+j],f)
                          nf += 1
                      end
                  end
...
#+begin_src julia
  function thinout(pop, fit, pareto, n::Int)
      ...
      pop[pareto[indices[end-n+1:end]]]
  end
#+end_src 
** utility functions
...
#+begin_src julia :tangle testminlp.jl
  ...
  a = MI(1.0, 1)
  b = MI(6.0, 6)
  # function to find a neigbouring solution
  function neighbour(s,a,b,f)
  ...
  bestx, bestz, bestg, pop = Fresa.solve(f, MI(1.0, 1), a, b; ngen=100, neighbourfn=neighbour)
...
#+begin_src julia :tangle testmultiobjective.jl
  using Fresa
  function multibojectiveproblem()
      nx = 2
      a = zeros(nx,1)
      b = ones(nx,1)
      x = rand(nx,1)
  ...
  @benchmark multibojectiveproblem()

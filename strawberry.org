...
| *Last revision* | {{{modification-time}}}       |
...
#+begin_src shell :results none :exports none
  scp strawberry.org strawberry.html strawberry_test3.png socrates.ucl.ac.uk:html.pub
#+end_src
...
- f :: handle for the objective function which has signature ~[z g] = f(x)~ where ~z~ is a vector of objective function values and ~g~ indicates whether a solution is feasible (~g == 0)~ or infeasible (~g > 0~).  The actual value of ~g~ for infeasible solutions is currently not considered but could, in the future, be used to rank solutions based on how infeasible they may be.  Therefore, it is good practice to have the value be a measure of constraint violation if you wish your code to be future proofed.  If your objective function is unconstrained, simply ensure that ~0~ is returned as the second entry in the return list.
...
#+begin_src octave :tangle strawberry_initialise.m
  function strawberry_initialise
    ...
    if isempty (strawberry_version)
      strawberry_version = '2016.11.15 15:50:05';
      mosd_initialise
      moga_initialise

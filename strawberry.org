...
#+begin_src octave :tangle strawberry_initialise.m
  function strawberry_initialise
    ...
    if isempty (strawberry_version)
      strawberry_version = '2016.10.17 08:27:10';
      mosd_initialise
      moga_initialise
...
- new solution :: generate a random solution in the search space
...
#+begin_src octave :tangle strawberry_newpopulation.m
  function [pop, nx, nz, nf, ninf] = strawberry_newpopulation (x0,npop,f,constrained,g)
    ...
    for i=2:npop
      xcell{i} = strawberry_randomsolution();
    endfor
...
#+begin_src octave :noweb yes :tangle strawberry.m
  if constrained
    [phi, nx, nz, nf, ninf] = strawberry_newpopulation (x0,npop,f,constrained,g);
  else
...
#+begin_src octave :noweb yes :tangle strawberry.m
  ...
  for p=1:npop                # we pick up to NPOP members to propagate
    ...
    if s > 0
      ...
      for r=1:nr
        newpop{n} = strawberry_neighbour(pop(1:nx,s), N(s));

...
* TODO [4/12] improvements or bugs                            :noexport:
...
#+begin_src octave :noweb yes :tangle strawberry.m
  ...
  if nz < 2
    ...
    best = pop(:,findpareto(pop(nx+1:nx+nz,:)));
  endif
  bestgen = gen;
...
#+begin_src octave :noweb yes :tangle strawberry.m
  ...
  elseif nz>1
    ...
    ## of the old best and the current population
    if isempty(best)
      best = pareto;
    else
      union = [best, pareto];
      best = union(:,findpareto(union(nx+1:nx+nz,:)));
    endif
...
#+begin_src octave :noweb yes :tangle strawberry.m
  if 0 == mod(gen,output) || 1 == gen
    ...
    if nz > 1
      ...
      printf(' %9d | %9.1f | %9d |', nfunctionevaluations, dtime, size(best,2));
    else
      if nx > 5
        indices = [1:4 nx+nz];
      else
...
#+begin_src octave :tangle strawberry_fitness.m
  function [fit, best] = strawberry_fitness(p,nx,nz,npop)
    ...
    if nz < 2
      ...
      best = p(:,findpareto(p(nx+1:nx+nz,:)));
    endif
  endfunction
#+end_src
...
#+begin_src octave :tangle strawberry_initialise.m
  function strawberry_initialise
    ...
    if isempty (strawberry_version)
      strawberry_version = '2016.11.28 15:56:44';
      mosd_initialise
      moga_initialise

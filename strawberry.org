...
[[file:./strawberry-20161130.zip][strawberry-20161130.zip]]
...
#+end_src
*** output statistics and population periodically
There are two types of periodic output.  The first, controlled by the ~output~ argument to the function, is a single line summary of the status of the current population.  This defaults to outputting a total of 50 lines over the full evolution.
...
#+begin_src octave
  if strawberry_output_population > 0 && 0 == mod(gen,strawberry_output_population)
    filename = sprintf('%s-%s.data', 'strawberry-population-', ...
...
#+begin_src octave :tangle strawberry_initialise.m
  function strawberry_initialise
    ...
    if isempty (strawberry_version)
      strawberry_version = '2017.02.04 11:26:19';
      mosd_initialise
      moga_initialise

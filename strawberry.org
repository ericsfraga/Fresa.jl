...
The code, suitably manipulated to be compatible with MATLAB, can be found here: call_distribution(). All code is copyright © 2016, Eric S Fraga, all rights reserved.  Permission is given to use this code for evaluation purposes. The code is made available with no warranty or guarantee of any kind.  Use at own risk.  The code from the above link includes some supplementary functions are required for the /Strawberry/ method (utility functions, multi-objective genetic algorithm (MOGA), and multi-objective steepest descent (MOSD)).

Please let [[mailto:e.fraga@ucl.ac.uk?subject=The%20Strawberry%20code][the author know]] if you download the code.  Further, if the /Strawberry/ code is used, please do let the authors know and the above publications must be cited in any research papers written.  Feedback, including bug reports, is most welcome.

** Upload to web site                                        :noexport:
#+begin_src shell :results none :exports none
  scp strawberry.html strawberry_test3.png socrates.ucl.ac.uk:html.pub
#+end_src
...
#+begin_src shell :results output raw :exports results
  ...
  do
      ...
      cat ${mfile} | sed -e 's/#/%/g' -e 's/endfunction/end/' -e 's/endfor/end/' -e 's/endif/end/' > ${dist}/${base}.m
  done
...
[[file:./strawberry-20160905.zip][strawberry-20160905.zip]]
...
#+begin_src octave :tangle strawberry_initialise.m
  function strawberry_initialise
    ...
    if isempty (strawberry_version)
      strawberry_version = "2016.09.05 15:50:42";
      mosd_initialise
      moga_initialise

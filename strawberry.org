...
- f :: handle for the objective function which has signature ~[z g] = f(x)~ where ~g~ is optional and represents the constraints.  If ~g~ is present, the problem is assumed to be constrained and feasible points are those for which ~g~ is 0 (false); infeasible points are those with a value of 1 (true).  It should be noted that any objective function value of 10^{20} is assumed to indicate an infeasible point even if there is no ~g~. 
...
| output              | ngen/50 |
|---------------------+---------|
There is no default for ~g~.  If present, it is used; if not, the problem is assumed to be unconstrained although the objective function could be defined to include a penalty function.  
*** Return values
...
#+begin_src octave :tangle strawberry_initialise.m
  function strawberry_initialise
    ...
    if isempty (strawberry_version)
      strawberry_version = '2016.11.11 08:39:48';
      mosd_initialise
      moga_initialise

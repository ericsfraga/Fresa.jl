...
*Breaking news*! An implementation of the plant propagation algorithm in the [[http://julialang.org/][Julia programming language]], known as [[file:fresa.org][Fresa]], is now available and under further development.  In all likelihood, further development of /Strawberry/ will be limited to bug fixes.
...
[[file:./strawberry-20161116.zip][strawberry-20161116.zip]]

* Version information

Major version log:

- November 2016 :: change of interface to Strawberry: the objective function must now return not only the objective function values (first entry in return list) but also the feasibility of the point in the search space (as second entry).
...
* TODO [4/11] improvements or bugs                            :noexport:
...
* strawberry -- the main function for the package
:PROPERTIES:
...
- f :: handle for the objective function which has signature
     : [z g] = f(x)
     where ~z~ is a vector of objective function values and ~g~ indicates whether a solution is feasible (~g == 0)~ or infeasible (~g > 0~).  The actual value of ~g~ for infeasible solutions is currently not considered but could, in the future, be used to rank solutions based on how infeasible they may be.  Therefore, it is good practice to have the value be a measure of constraint violation if you wish your code to be future proofed.  If your objective function is unconstrained, simply ensure that ~0~ is returned as the second entry in the return list.
...
#+begin_src octave :noweb yes :tangle strawberry.m
  if nz > 1
    ...
    if sum(fitindices) < size(pop,2)
      printf('Removing %d solutions from population of %d.  \n', size(pop,2)-sum(fitindices), size(pop,2))
      pop = pop(:,fitindices);
    endif
  endif
...
#+end_src
* distance -- the default distance function for real valued compact search domains
...
#+end_src
* fitness -- compute fitness of solutions, both single and multiple criteria
...
#+begin_src octave :tangle strawberry_fitness.m
  function [fit, best] = strawberry_fitness(p,nx,nz,npop)
    ...
    if sum(indexfeasible) > 0
      feasible =(pop,1:k,indexfeasible)
      feasiblefit = strawberry_vectorfitness(nz, p(n+1,indexfeasible));
    ...
    if sum(indexinfeasible) > 0
      ...
      fit[indexinfeasible] = vectorfitness(m, p(n+1:n+nz,indexinfeasible))/factor;
    end
...
#+begin_src octave :tangle strawberry_initialise.m
  function strawberry_initialise
    ...
    if isempty (strawberry_version)
      strawberry_version = '2016.11.24 10:16:24';
      mosd_initialise
      moga_initialise
...
#+end_src
* isdiverse -- identify solutions which are sufficiently different
...
#+end_src
* neighbour -- default function (compact real valued domain) to generate a neighbouring (fitness dependent) solution
...
#+end_src
* prune -- remove non-diverse members of the population to avoid stagnation
...
#+end_src
* random solution -- generate a new solution in the search space
...
#+end_src
* runners -- calculate number of runners to generate as a function of fitness
